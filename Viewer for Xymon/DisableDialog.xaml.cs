...
{
    public sealed partial class DisableDialog : ContentDialog
    {
        public static readonly DependencyProperty TextProperty = DependencyProperty.Register(
            "Text", typeof(string), typeof(DisableDialog), new PropertyMetadata(default(string)));

        ObservableCollection<object> sel;
        public DisableJob job;
        private List<string> timeUnits = new List<string>();

        public DisableDialog(ObservableCollection<object> selected)
        {
            InitializeComponent();
            if (Settings.disableUntilGreen) rb3.Visibility = Windows.UI.Xaml.Visibility.Visible;
            else rb3.Visibility = Windows.UI.Xaml.Visibility.Collapsed;

            Status.createCaseStatus = -1; // -1: No attempt, 0: Failure, 1: Success, 2: Found existing cases
            SelectionGrid.UseSystemFocusVisuals = true;
            ...
            timeUnitCombo.SelectedItem = "hours";

            sel = selected;
            job = new DisableJob(selected);
            DataContext = job;

            if (job.message == null)
            {
                //job.message = job.t.Result;
                    //await t;
            }
        ...
        private void ContentDialog_PrimaryButtonClick(ContentDialog sender, ContentDialogButtonClickEventArgs args)
        {
            ...
            xymonConnect.xymonDisableAsync(job);
        }
        ...
        private void CopyButton_Tapped(object sender, TappedRoutedEventArgs e)
        {
            ...
            foreach (DisableTarget t in job.targets)
            {
        ...
        private async void CaseButton_Tapped(object sender, TappedRoutedEventArgs e)
        {
            ...
            Match m = Regex.Match(t.Result, p);
            if (!m.Success)
            {
            ...
            else
            {
                ...
                {
                    Status.createCaseStatus = 1;
                    Match refMatch = Regex.Match(t.Result, String.Format("({0})(?!.*\\1)", Settings.casePattern));
                    if (!refMatch.Success)
                    {
                ...
                else if (m.Groups["exists"].Success)
                {
                    ...
                    MatchCollection refMatches = Regex.Matches(t.Result, String.Format("({0})(?!.*\\1)", Settings.casePattern));
        ...
        {
            if (Settings.verboseLog) Debug.WriteLine("Disable targets selection changed");
            //if (String.IsNullOrEmpty(Settings.createCaseURL))
            if (Settings.createCaseURL != "" && Settings.createCaseURL != null)
            {
                if (Status.createCaseStatus == -1)
                {
                    ...
                    if (SelectionGrid.SelectedItems.Count == 1)
                    {
                        ...
                        var target = SelectionGrid.SelectedItems.First() as DisableTarget;
        ...
        public async void OpenErrorDialog(object sender, RoutedEventArgs e, string error)
        {
            ...
            catch (Exception generalException) { Debug.WriteLine(generalException); }
